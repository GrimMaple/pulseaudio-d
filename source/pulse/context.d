 ...
 * 0.9.15 */
alias pa_context_event_cb_t = void function (pa_context* c, const(char)* name, pa_proplist* p, void* userdata);
...
uint pa_context_get_protocol_version (const(pa_context)* c);

/** Return the protocol version of the connected server.
 * Returns PA_INVALID_INDEX on error. */
uint pa_context_get_server_protocol_version (const(pa_context)* c);
...
pa_operation* pa_context_proplist_remove (pa_context* c, const(char*)* keys, pa_context_success_cb_t cb, void* userdata);
...
uint pa_context_get_index (const(pa_context)* s);
